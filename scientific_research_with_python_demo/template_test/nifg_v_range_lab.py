...
success_rate = np.zeros((91 * 170, 3))
r = 0
for j in range(len(Nifg_range)):
    Nifg = Nifg_range[j]
    print("Nifg = ", Nifg_range[j])
    ...
    time_baseline = np.arange(1, Nifg + 1, 1).reshape(1, Nifg) * dt_orig
    ...
    for i in range(len(v_orig)):
        ...
        success_rate[r, 1] = Nifg_range[j]
        success_rate[r, 2] = success / iteration
        r += 1


np.savetxt("/data/tests/jiaxing/scientific_research_with_python_demo/scientific_research_with_python_demo/data_save/nifg_Vsuccess_lab_1.csv", success_rate, delimiter=",")

...
for j in range(len(Nifg_orig)):
    ...
    time_baseline = np.arange(1, Nifg + 1, 1).reshape(1, Nifg) * dt_range
    ...
    np.savetxt("/data/tests/jiaxing/scientific_research_with_python_demo/scientific_research_with_python_demo/data_save/Nifgsuccess_lab%s_1.csv" % j, success_rate)
    print("success_rate_save !")
    T2 = time.perf_counter()

    # dp.bar_plot(v_orig * 1000, success_rate, "Nifg=10,SNR=70db,dt=12", "snr_v_test5", 0.001 * 1000, "v[mm/year]")
    dp.line_plot(v_orig * 1000, success_rate, "n=10deg,dt=36,nifg=%s" % Nifg_orig[j], "Nifg_lab%s_1" % j, "v[mm/year]")

...
success_rate = np.zeros(len(dt_range))
...
for i in range(len(dt_range)):
    ...
    while iteration < 1000:
        ...
        while count <= 10 and data_set["velocity"]["step_orig"] > 1.0e-8 and data_set["height"]["step_orig"] > 1.0e-4:
            ...
            count += 1
        if abs(est_param["velocity"] - v_orig) < 0.0005:
            success += 1
        ...
        # else:
        # print(est_param)
    with open("/data/tests/jiaxing/scientific_research_with_python_demo/scientific_research_with_python_demo/data_save/dT_test4.csv", "a") as f:
        # 按列追加保存
...
np.savetxt("/data/tests/jiaxing/scientific_research_with_python_demo/scientific_research_with_python_demo/data_save/dTsuccess_test4.csv", success_rate)
T2 = time.perf_counter()
print("程序运行时间:%s秒" % (T2 - T1))
# dp.bar_plot(v_orig * 1000, success_rate, "Nifg=10,SNR=70db,dt=12", "snr_v_test5", 0.001 * 1000, "v[mm/year]")
dp.line_plot(12 * dt_range, success_rate, "n=20deg,v=0.005,nifg=30", "dT_test4", "dT[days]")

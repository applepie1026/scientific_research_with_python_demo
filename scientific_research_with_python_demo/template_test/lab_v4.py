...
for i in range(len(v_orig)):
    ...
    while iteration < 1000:
        ...
        while count <= 10 and data_set["velocity"]["step_orig"] > 1.0e-8 and data_set["height"]["step_orig"] > 1.0e-4:
            ...
            count += 1
        if abs(est_param["velocity"] - v) < 0.0005 and abs(est_param["height"] - h) < 0.05:
            success += 1
        ...
        # else:
        # print(est_param)
    with open("/data/tests/jiaxing/scientific_research_with_python_demo/scientific_research_with_python_demo/data_save/V_test7.csv", "a") as f:
        # 按列追加保存
...
np.savetxt("/data/tests/jiaxing/scientific_research_with_python_demo/scientific_research_with_python_demo/data_save/Vsuccess_test7.csv", success_rate)
T2 = time.perf_counter()
print("程序运行时间:%s秒" % (T2 - T1))
# dp.bar_plot(v_orig * 1000, success_rate, "Nifg=10,SNR=70db,dt=12", "snr_v_test5", 0.001 * 1000, "v[mm/year]")
dp.line_plot(v_orig * 1000, success_rate, "n=10deg,dt=12,nifg=30", "V_test7", "v[mm/year]")

...
pseudo_obs = np.array([[0, 0]]).T
noise_level = 10
param_name = ["height", "velocity"]
normal_baseline = np.random.randn(1, Nifg) * 333
# print(normal_baseline)
time_baseline = np.arange(1, Nifg + 1, 1).reshape(1, Nifg) * 36  # 减小重访周期 dt 能明显改善结果
...
a_hat = (phase_obs - v2ph * v_orig - h2ph * h_orig) / (2 * np.pi)
...
afixed = ils.main(a_hat, Q_a_guess, method=1, ncands=2)[0]
# print(afixed)
...
afixed_new, sqnorm, Ps, Qzhat, Z, nfixed, mu = ils.main(a_hat, Q_a_guess_new, 1)
